 ...
 * @since      1.0.0
 */
class myshare_pro_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-myshare_pro-recent widget_posts_thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'myshare-pro' )
		);

		// Create the widget.
		parent::__construct(
			'myshare_pro-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'myshare-pro' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'myshare_pro_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'myshare_pro_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'myshare_pro_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'myshare-pro' ),
			'limit'     => 5,
			'show_date' => true
		);
