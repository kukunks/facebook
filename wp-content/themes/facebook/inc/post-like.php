...
function sl_enqueue_scripts() {
	...
	wp_localize_script( 'simple-likes-public-js', 'simpleLikes', array(
		...
		'like' => __( 'Like', 'myshare-pro' ),
		'unlike' => __( 'Unlike', 'myshare-pro' )
	) ); 
}
...
function process_simple_like() {
	...
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( __( 'Not permitted', 'myshare-pro' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = __( 'Unlike', 'myshare-pro' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = __( 'Like', 'myshare-pro' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = __( 'Like', 'myshare-pro' );
	if ( is_numeric( $like_count ) && $like_count > 0 && $like_count < 2 ) { 
		$number = sl_format_count( $like_count ) . ' ' . 'Like';
	} elseif ( is_numeric( $like_count ) && $like_count > 1 ) {
		$number = sl_format_count( $like_count ) . ' ' . 'Likes';
	} else {
		$number = $like_text;
	}
...
function show_user_likes( $user ) { ?>        
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'myshare-pro' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php _e( 'You do not like anything yet.', 'myshare-pro' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
